...
from consumer.consumer_lambda import (
...
def test_maps_attribute_to_internal():
    ...
    result = map_attribute_to_internal(attribute)
...
def test_map_attribute_returns_none_for_none():
    assert map_attribute_to_internal(None) is None
...
def test_maps_extended_attributes_to_internal():
    ...
    result = map_extended_attributes_to_internal(ext)
...
def test_map_extended_attributes_returns_none_for_none():
    assert map_extended_attributes_to_internal(None) is None
...
def test_same_service_request_produces_same_hash():
    ...
    result_1 = map_to_internal(sr)
    result_2 = map_to_internal(sr)
...
def test_different_service_requests_produce_different_hashes():
    ...
    result_1 = map_to_internal(sr_1)
    result_2 = map_to_internal(sr_2)

    assert result_1.event_hash != result_2.event_hash


def test_returns_none_when_service_request_is_none():
    assert map_to_internal(None) is None
...
def test_returns_none_when_internal_mapping_raises(monkeypatch):
    ...
    monkeypatch.setattr("consumer.consumer_lambda.hashlib.sha256", _raise)
    result = map_to_internal(sr)
